...
public class Server {
	...
	public int[] gameList(int[] msg) throws SQLException {
		String comm="SELECT g.Name,g.GID FROM Game g,InGame i WHERE i.GID=g.GID AND i.Username="+extractUsername(msg)+";";
		ResultSet rs = stmt.executeQuery( comm );
		int totallen=0;
	...
	public int[] gameSeekList(int[] msg) throws SQLException {
		String comm="SELECT g.Name,g.GSID FROM GameSeek g,Seeking i WHERE i.GSID=g.GSID AND i.Username="+extractUsername(msg)+";";
		ResultSet rs = stmt.executeQuery( comm );
		int totallen=0;
		ArrayList<int[]> taco=new ArrayList<int[]>();
